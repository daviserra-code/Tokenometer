...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const state = createProxyState(req, "OpenAI", "/api/proxy/openai/chat/completions");
  ...
  const auth = await authProxy(req, "OpenAI");
  ...
  try {
    ...
    return jsonProxyError(state, 502, "OpenAI upstream request failed.", {
  ...
  if (upstreamRes.ok) {
    try {
      ...
      console.error("OpenAI metering failed:", error);
    }
  }

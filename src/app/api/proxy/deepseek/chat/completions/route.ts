...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const state = createProxyState(req, "DeepSeek", "/api/proxy/deepseek/chat/completions");
  ...
  const auth = await authProxy(req, "DeepSeek");
  ...
  try {
    ...
    return jsonProxyError(state, 502, "DeepSeek upstream request failed.", {
  ...
  if (upstreamRes.ok) {
    try {
      ...
      console.error("DeepSeek metering failed:", error);
    }
  }
